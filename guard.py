...
class Guard(Entity):
    ...
    def __init__(self, position=(0,0,0), cost=int, power=int):
        super().__init__(
            parent=scene,
            model="Assets/guard.obj",
            texture="Assets/guard.jpeg",
            #color="#1A2421",
            scale=3,
            origin=(0,0,0),
            position=position,
    ...
    def input(self, key):
        if held_keys['shift'] and self.hovered and Guard.currently_dragged_entity is None:
            if mouse.left:
                Guard.currently_dragged_entity = self
                self.dragging = True
        elif key == 'left mouse up':
            if self.dragging:
                self.dragging = False
                Guard.currently_dragged_entity = None
